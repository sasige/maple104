...
var returnTo = new Array(130000210,104020120);
...
function timeOut() {
	var iter = em.getInstance("SDtoJ1").getPlayers().iterator();
	while (iter.hasNext()) {
		var player = iter.next();
		player.changeMap(docked, docked.getPortal(0));
		em.getInstance("SDtoJ1").unregisterPlayer(player);
	}
	em.setProperty("isRiding","false");
	em.disposeInstance("SDtoJ1");
}
