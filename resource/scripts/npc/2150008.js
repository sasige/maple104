...
function start() {
	status = -1;
	SDtoJ3 = cm.getEventManager("SDtoJ3");
	action(1, 0, 0);
}
	...
	if(mode == -1) {
		...
		if (status == 0) {
			for(var i=0; i < menu.length; i++) {
				if(cm.getChar().getMapId() == 310000010 && i < 1) {
			...
			if(cm.getChar().getMapId() == 251000000) {
				...
				cm.sendSimple("嗯……你是说……你想离开埃德尔斯坦？前往金银岛魔法密林？路程所需的时间大约是#b2分钟#k，费用用1000金币。\r\n" + display);
			}
		} else if(status == 1) {
			if(selection == 2) {
				...
				if(cm.getMeso() < cost[selection]) {
					...
					if(cm.getChar().getMapId() == 251000000) {
						...
						if(SDtoJ3.getProperty("isRiding").equals("false")) {
							cm.gainMeso(-cost[selection]);
							SDtoJ3.newInstance("SDtoJ3");
							SDtoJ3.setProperty("myRide",selection);
							SDtoJ3.getInstance("SDtoJ3").registerPlayer(cm.getChar());
							cm.dispose();
						} else {
							cm.gainMeso(-cost[3]);
							cm.warp(250000100);
