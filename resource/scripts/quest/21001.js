...
function start(mode, type, selection) {
	...
	qm.sendNext("*Sob* Aran has declined my request!");
        ...
        if (mode == 1)
            ...
            status--;
		if (status == 0)
			qm.sendYesNo("嗯嗯……吓死我了……快，带我到赫丽娜那边去！");
		else if (status == 1) {
			qm.forceStartQuest();
...
function end(mode, type, selection) {
	...
	qm.sendNext("孩子怎么样？请给我的孩子。");
		...
		if (status == 0)
			...
			qm.sendNextPrev("赶快上船！已经没时间了！");
		else if (status == 3)
			qm.sendNext("没错。现在不是感伤的时候。黑魔法师的气息越来越近！似乎他们已经察觉方舟的位置，得赶紧启航，不然就来不及了！");
		else if (status == 4)
			qm.sendNextPrev("立刻出发！");
		else if (status == 5)
			qm.sendNextPrev("战神！请你也上船吧！我们理解你渴望战斗的心情……不过，现在已经晚了！战斗就交给你的那些同伴吧，和我们一起去金银岛吧！");
		else if (status == 6)
			qm.sendNext("不行！");
		else if (status == 7) {
			qm.sendNext("赫丽娜，你先出发去金银岛。一定要活着，我们一定会再见的。我要和同伴们一起同黑魔法师战斗！");
		} else if (status == 8) {
			//qm.clearAranPolearm();
