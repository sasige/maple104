...
function monsterstart() {
	...
	while (iter.hasNext()) {
		var eim = iter.next();
		var mob = net.sf.odinms.server.life.MapleLifeFactory.getMonster(9300139);
        var overrideStats = new net.sf.odinms.server.life.MapleMonsterStats(); 
        var map = eim.getMapFactory().getMap(mapId); 
        var xRand = Math.floor(Math.random() * 150)+25;
        overrideStats.setHp(700000); 
        overrideStats.setExp(20000); 
        overrideStats.setMp(50000); 
        mob.setOverrideStats(overrideStats); 
        mob.setHp(700000);
        eim.registerMonster(mob);
        map.spawnMonsterOnGroudBelow(mob, new java.awt.Point(68, 209));
        map.broadcastMessage(net.sf.odinms.tools.MaplePacketCreator.serverNotice(5, "法兰肯已经被召唤! 请大家小心!!"));
	}
}
