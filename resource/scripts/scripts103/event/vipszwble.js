...
function monsterstart() {
	...
	while (iter.hasNext()) {
		var eim = iter.next();
			var mob = net.sf.odinms.server.life.MapleLifeFactory.getMonster(9800066);
			var overrideStats = new net.sf.odinms.server.life.MapleMonsterStats(); 
			var map = eim.getMapFactory().getMap(mapId); 
			var xRand = 330-Math.floor(Math.random() * 650);
			var hprand = 350000000;
			overrideStats.setHp(hprand); 
			overrideStats.setExp(18400); 
			overrideStats.setMp(1500000); 
			mob.setOverrideStats(overrideStats); 
			mob.setHp(hprand);
			eim.registerMonster(mob);
			
map.spawnMonsterOnGroudBelow(mob, new java.awt.Point(2214, 823));
 map.broadcastMessage(net.sf.odinms.tools.MaplePacketCreator.serverNotice(5, " 大王蜈蚣从地下爬了出来!请在规定时间内打到它!"));
	}
}
