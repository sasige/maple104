...
function playerEntry(eim, player) {
	...
	//player.getClient().getSession().write(org.tools.MaplePacketCreator.getClock(1800));
}
...
function timeOut(eim) {
	...
	while (iter.hasNext()) {
		var eims = iter.next();
		if (eims.getPlayerCount() > 0) {
			var pIter = eims.getPlayers().iterator();
			while (pIter.hasNext()) {
				playerExit(eims, pIter.next());
			}
		}
		eims.dispose();
	}
