    ...
    if (mode == -1) {  
        ...
        if (status == 0) {
            ...
            if (cm.getChar().getBossLog('fszk') < 1)
            {
                ...
                cm.serverNotice("[粉色地带]:玩家"+cm.getChar().getName()+"已经进入挑战粉色扎昆地带]"+cm.GetSN()+"☆☆☆☆☆☆☆.");
cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.odinms.tools.MaplePacketCreator.serverNotice(17,cm.getC().getChannel(),"[粉色地带]" + " : 玩家"+cm.getChar().getName()+"已经进入挑战粉色扎昆地带]"+cm.GetSN()+"☆☆☆☆☆☆☆.",true).getBytes());

                cm.dispose();
            }
            else
            {
                cm.sendOk("#r你每天只能允许进入1次 ! 时间未到.");
                mode = 1;
                status = -1;
            }
