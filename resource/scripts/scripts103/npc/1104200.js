...
cm.dispose();
if (selection == 0) {
        if (!cm.haveItem(4031466,1)) {
        cm.sendOk("很抱歉本关只可以召唤一次·");
        } else if (!cm.haveItem(4031466,1)) {
        }else{
	cm.gainItem(4031466,-1);
        cm.summonMob(8300006,2100000000, 40000, 3);     
cm.serverNotice("『御龙终结者(宠物)副本』：【"+ cm.getChar().getName() +"】带领他的队友开始挑御龙终结者(宠物)BOSS【御龙终结者(宠物)副本 】！");
cm.getC().getChannelServer().broadcastPacket(net.sf.odinms.tools.MaplePacketCreator.serverNotice(0x10,cm.getC().getChannel(),"『御龙终结者(宠物)』"+" : "+"我擦、居然你们敢挑战我、不想活着出去了！！"))
	cm.dispose();}
} else if (selection == 1) {
...
cm.serverNotice("『会员公告』：【"+ cm.getChar().getName() +"】带领他的队友开始挑战地狱大公终极BOSS【地狱船长  】！"); 
	...
	cm.dispose();}
        ...
        if (!cm.haveItem(4001401,2)) {
        cm.sendOk("抱歉，你没有2张#v4001401#求败#e#r(求败)");
        } else if (!cm.haveItem(4001401,2)) {
	cm.gainItem(4001401,-2);
        }else{
	cm.gainItem(4001401,-2);
        cm.warp(922020000,0); 
         cm.dispose();
