...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("你好,我是豫园美发店的店长#p9310016#!如果你有#b豫园美发店高级会员卡#k或#b豫园染色高级会员卡#k,你就放心的把发型交给我,我会让你满意的.那么你要做什么?请选择吧!.\r\n#L1#改变发型(使用#b豫园美发店高级会员卡#k)#l\r\n#L2#染色(使用#b豫园染色高级会员卡#k)#l");						
		} else if (status == 1) {
			 if (selection == 1) {
				beauty = 1;
				hairnew = Array();
				...
				cm.sendStyle("我的手艺是拜师的来。如果你有#b豫园美发店高级会员卡#k就可以让你换一个发型,请选择喜欢的发型吧.", hairnew,5150035);
			} else if (selection == 2) {
				beauty = 2;
				haircolor = Array();
				...
				cm.sendStyle("我可以改变你的发色,让它比现在看起来漂亮. 你为什么不试着改变它下? 如果你有#b豫园染色高级会员卡#k,我将会帮你改变你的发色,那么选择一个你想要的新发色吧!", haircolor,5151030);			}
		} else if (status == 2){
			if (beauty == 1){
