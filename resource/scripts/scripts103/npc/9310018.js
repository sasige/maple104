...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("欢迎光临！欢迎来到我们豫园护肤中心。你是不是希望拥有像我一样健康、美丽的肌肤呢？如果你有#b#t5153003##k我们可以为你精心护理肌肤。请相信我们的能力，怎么样要不要试一试？\r\n\#L2##b改变肤色#k(使用#b#t5153003##k)");
		} else if (status == 1) {
			if (selection == 1) {
				cm.dispose();
			} else if (selection == 2) {
