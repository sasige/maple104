function start() {
	if (cm.getChar().getMapId() == 749020900){
...
cm.dispose();
if (selection == 0) {
        if (!cm.haveItem(3994039,1)) {
        cm.sendOk("抱歉，你没有1张#v3994039#无法为你开启#e#r");
        }else{
	cm.gainItem(3994039,-1);
        cm.summonMob(9400751,2100000000, 40000, 1);     
        cm.summonMob(9400764,1400000000, 40000, 1); 
        cm.summonMob(9400754,20000, 40000, 1); 
        cm.summonMob(9400752,2100000000, 40000, 1); 
        cm.summonMob(9400753,2100000000, 40000, 1); 
        cm.summonMob(9400763,21000, 40000, 1);
cm.serverNotice("『BOSS系统』：【"+ cm.getChar().getName() +"】欢迎来到bigbang蛋糕副本··~!");
cm.getC().getChannelServer().broadcastPacket(net.sf.odinms.tools.MaplePacketCreator.serverNotice(0x10,cm.getC().getChannel(),"『蛋糕BOSS』"+" : "+"欢迎来到bigbang蛋糕副本！.~!"))
	cm.dispose();}
} else if (selection == 1) {
