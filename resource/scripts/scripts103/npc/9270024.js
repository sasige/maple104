...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("嘿~！你好，欢迎来到#b中心商务区整形会员中心#k。如果你有#b#t"+card+"##k，我可以为你进行整形手术。\r\n\#L2##b使用#i"+card+"#进行整形手术#k#l");
			} else if (status == 1) {
			if (selection == 1) {
				cm.dispose();
			} else if (selection == 2) {
				...
				cm.sendStyle("我可以改变你的脸型,让它比现在看起来漂亮. 你为什么不试着改变它下? 如果你有#b#t"+card+"##k,我将会帮你改变你的脸型,那么选择一个你想要的新脸型吧!", facenew,5152038);
			}
		} else if (status == 2){	
			cm.dispose();
