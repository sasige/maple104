var psrw1 = new Array(3, 3, 3, 4, 5, 2, 2, 2, 2, 2, 2, 2, 4, 4, 5, 2, 2);
var rand1 = Math.floor(Math.random() * psrw1.length);
function start() {
	if (cm.getChar().getMapId() == 980042200){
	    if(cm.countMonster()>0){
		 cm.sendSimple ("您正在挑战BOSS,请选把它消灭了再来找我召唤!!如果您想放弃挑战,我可以把你送回市场!\r\n    #L8##r回到市场#l");
	    }else{
   		 cm.sendSimple ("god，你居然征服了魔女的玩具熊，真心佩服你呢~~\r\n#L0#领取奖励");
	    }
	} else {
...
if (selection == 0) {
        ...
        cm.sendOk("恭喜你获得了" + psrw1[rand1] + "个#v4000313#"); 
        //cm.getC().getChannelServer().broadcastPacket(Packages.tools.MaplePacketCreator.serverNotice(0x10, cm.getC().getChannel(), "[征战魔女塔<普通>]" + " : " + "恭喜" + cm.getChar().getName() + "通过了魔女塔考验，获得了大量的黄金枫叶"));
	cm.dispose();
} else if (selection == 1) {
