...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("你好,我是美发店的助手!由于马来西亚会员卡缺货，所以用#b中心商业区会员卡代替#k如果你有#b中心商业区美发店普通会员卡#k或#b中心商业区染色普通会员卡#k,你就放心的把发型交给我,我会让你满意的.那么你要做什么?请选择吧!\r\n#L1#改变发型(使用#b中心商业区美发店普通会员卡#k)#l\r\n#L2#染色(使用#b中心商业区染发普通会员卡#k)#l");								
			} else if (status == 1) {
			if (selection == 0) {
				beauty = 0;
				cm.sendSimple("那么你想购买哪种会员卡呢?\r\n#L0##b中心商业区美发店普通会员卡#k,需要#r" + hairprice + "#k金币#l\r\n#L1##b中心商业区染发普通会员卡#k,需要#r" + haircolorprice + "#k金币#l");
			} else if (selection == 1) {
				beauty = 1;
				hairnew = Array();
				...
				cm.sendYesNo("如果你有#b中心商业区美发店普通会员卡#k,那么我将帮你随机改变一种发型,你确定要改变发型吗?");
			} else if (selection == 2) {
				beauty = 2;
				haircolor = Array();
				...
				cm.sendYesNo("如果你有#b中心商业区染发普通会员卡#k,那么我将帮你随机改变一种发色,你确定要改变发色吗?");
			}
		}
		else if (status == 2){
			cm.dispose();
