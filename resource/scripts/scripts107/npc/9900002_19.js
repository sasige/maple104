...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getHyPay(4) >= 0) {
            cm.sendYesNo("这里是充值抽奖专区。我背包里面包含了必成卷轴、大PB、椅子、永恒、140所有装备哦。你确定要抽奖吗？");}
    } else if (status == 1) {
        if (cm.delPayReward(0) > 0) {
            var item;
            var chance = Math.floor(Math.random() * 30);
            if (chance >= 0 && chance <= 8) {
                cm.gainExp( + 500000000);
                cm.sendOk("恭喜你获得了5E经验值。");
		cm.worldMessage("恭喜玩家" + cm.getChar().getName() + "在充值大转盘(30)中吸5E经验。" );
                cm.dispose();
            } else if (chance >= 9 && chance <= 18) {
                cm.gainMeso( + 6666666);
                cm.sendOk("恭喜你获得了666W金币。");
		cm.worldMessage("恭喜玩家" + cm.getChar().getName() + "在充值大转盘(30)中无意拿到666W支票。" );
                cm.dispose();
