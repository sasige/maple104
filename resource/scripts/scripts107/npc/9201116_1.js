...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            status++;
        if (status == 0) {
            cm.sendNext("伟大的#b#h ##k，我是本服唯一的强化装备使者，有什么需要我效劳的么？");
            ...
            if (selection==0){//正义币
				fstype=1;
				cm.sendNext("你目前选择的是#r增加装备砸卷次数#k\r\n这项功能目前需要手续费用5000W冒险币+2个正义币\r\n#b注意,50%机率成功#k,当然失败会返回你一半的手续费费用");
            }else if (selection==2){//黄金枫叶
				fstype=3;
				cm.sendNext("你目前选择的是#r增加装备砸卷次数#k\r\n这项功能目前需要手续费用5000W冒险币+1个黄金枫叶\r\n#b注意,50%机率成功#k,当然失败会返回你一半的手续费费用");
            }else if (selection==3){//彩色枫叶
				fstype=4;
				cm.sendNext("你目前选择的是#r随即增加装备全属性(1~30)#k\r\n这项功能目前需要手续费用5000W冒险币+30个彩色枫叶\r\n#b注意,50%机率成功#k,当然失败会返回你一半的手续费费用\r\n什么是随即增加全属性呢？就是选择要加的装备后，会随即给这装备加全属性(1~30)，全凭人品哟");
