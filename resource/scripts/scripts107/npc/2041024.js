...
function action(mode,type,selection) {
	if (mode == -1) {
		...
		status = 0;
		cm.sendSimple("嘿，我的墓碑。在某些交易等件，我可以给你一部分的裂纹尺寸，或者如果你有三件，锻造成一个完成后 #b一张裂纹尺寸#k. So, 你想做什么？\r\n\r\n#b#L0#贸易一块裂纹尺寸 A#l\r\n#L1#贸易一块裂纹尺寸 B#l\r\n#L2#贸易一块裂纹尺寸 C#l\r\n#L3#打造一个完整的块裂维度#l\r\n#L4#查看排名的速度#l");
	} else if (status == 0) {
		if (selection == 0) {
			if (cm.haveItem(4000147,50) && cm.haveItem(4000132,50)) { //50 sealed teddy bears and 50 ghost pirate keys
				status = 1;
				cm.sendYesNo("呵！看起来你有物品。所以，你确定你想贸易 #r50 Sealed Teddy Bears#k and #r50 Ghost Pirate Keys#k for a #rPiece of Cracked Dimension A#k?");
			} else {
				cm.sendOk("如果你给我一些东西,我会给你一个#rPiece of Cracked Dimension A#k. They can be found from the monsters around here. The items are:\r\n\r\n#i4000147# x50\r\n#i4000132# x50");
				cm.dispose();
			}
		} else if (selection == 1) {
		...
		if (mode == 1) {
			...
			cm.sendOk("那就这样吧!如果你想要的东西,是没有限制的多少这些我可以。给我我那些东西。");
			cm.dispose();
		} else {
			cm.sendNext("我可以让你随时做那些东西。");
			cm.dispose();
		}
	} else {
		cm.dispose();
