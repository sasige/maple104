...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection == 0) {
            if (cm.getPlayer().getLevel() > 1) {
                cm.maxStats();
                cm.sendOk("我已经帮您加满属性了 祝你游戏愉快");
            }
            cm.dispose();
        } else if (selection == 7) {
            if (ccm.getPlayer().getLevel() > 1) {
            ...
            cm.dispose();
        } else if (selection == 2 && cm.getPlayer().getLevel() > 1) {
            var avail = "";
            for (var i = -1; i > -199; i--) {
        ...
        } else if (selection == 3 && cm.getPlayer().getLevel() > 1) {
    ...
    } else if (status == 2 && cm.getPlayer().getLevel() > 1) {
    ...
    } else if (status == 3 && cm.getPlayer().getLevel() > 1) {
        statsSel = selection;
    ...
    } else if (status == 4 && cm.getPlayer().getLevel() > 1) {
        ...
        cm.dispose();
    } else if (status == 10 && cm.getPlayer().getLevel() > 1) {
        if (selection == 0) {
        ...
        status = 0;
    } else if (status == 11 && cm.getPlayer().getLevel() > 1) {
