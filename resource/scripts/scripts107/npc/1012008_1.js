                ...
                if (jilv == 0) {} else if (jilv != 0) {
                    if (cm.getPlayer().getCSPoints(1) < cost) {
                        ...
                        if (jiaru == 0) {
                            costa = cost * jilv
                            cm.gainNX(costa);
                            //cm.getC().getChannelServer().broadcastPacket(Packages.tools.MaplePacketCreator.serverNotice(0x12, cm.getC().getChannel(), "[博彩公告]" + " : 恭喜玩家：" + cm.getPlayer().getName() + " 在金币博彩中赢得" + costa + "点卷。"));
