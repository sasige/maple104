	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (selection == 1) {
				...
				typed=6;
				var selStr="合成B级星岩石到A级,需要付2000消费币\r\n合成星岩石成功率为50%,合成失败后会返回1个星岩石,所以要做好心里准备哟\r\n";
				for (var i = 0; i < hccitem.length; i++) {//B合成A需要多加2000消费币
						selStr += "#b#L" + i + "##r#z" + hcaitem[i] + "##k需要2个#b#z" + hcbitem[i] + "#\r\n";
					}
				cm.sendSimpleS(selStr,2);
			} else if (selection == 7) {
				typed=7;
				var selStr="合成A级星岩石到S级,需要付10000消费币,合成成功率100%\r\n";
				for (var i = 0; i < hccitem.length; i++) {//A合成S需要多加10000消费币
