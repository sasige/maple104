...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		im.worldMessage("恭喜玩家" + im.getChar().getName() + "在充值大转盘(300面值)中吸取20E经验。" );
                im.dispose();
            } else if (chance >= 9 && chance <= 19) {
                im.gainMeso( + 100000000);
                im.sendOk("恭喜你获得了1E金币。");
		im.worldMessage("恭喜玩家" + im.getChar().getName() + "在充值大转盘(300面值)中无意拿到1E支票。" );
                im.dispose();
            } else {
                ...
                item = im.gainGachaponItem(itemList[Math.floor(Math.random() * itemList.length)], 1, "充值大转盘(300)");
            }
            if (item != -1) {
                im.sendOk("你获得了 #b#t" + item + "##k.");
            } else {
                im.sendOk("你确实充值到300了吗?如果是,请你确认在背包的装备,消耗,其他窗口中是否有一格以上的空间?");
            }
        } else {
            im.sendOk("xx错误");
        }
