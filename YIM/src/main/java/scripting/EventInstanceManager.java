...
public class EventInstanceManager {
  ...
  //  private MapleCharacter chr;
    ...
    public void Maplewingx(String Text, int typedd) {
        for (MapleCharacter chr : getPlayers()) {
           chr.getMap().startMapEffect(Text, typedd);
                    }
	
    }
    
    public void Maplewingxw(String Text, int typedd) {
        MaplePacketCreator.startMapEffect((new StringBuilder()).append("  ").append(Text).toString(), typedd, true);
    }
    
    
    public void startmapitem(MapleCharacter players)
	{
		try
		{
			MapleMap map = players.getMap();
			MapleItemInformationProvider iil = MapleItemInformationProvider.getInstance();
			int array[] = {
				1102149, 1102148, 2044103, 2044003, 1122000, 2043805, 2043703, 2043303, 2043203, 1432038,
				1432039, 1432042, 1302063, 1302059, 1302071, 1302049, 1302049, 1302106, 4001113, 4001114,
				2022175, 2022176, 2022178, 2022183, 2022112, 2022028, 5220000, 1472054, 1422011, 1422034,
				1422029, 1402053, 1402029, 1402051, 1402046, 1002677, 1002418, 1002749, 1102040, 1102041,
				1102042, 1102043
			};
			int rand = (int)Math.floor(Math.random() * 41D);
			map.spawnItemDropdir(players, players, iil.getEquipById(array[rand]), true, false);
		}
		catch (Exception e)
		{
			Fake(e);
    	...
    	public void delmapitem(MapleCharacter players)
	{
		try
		{
			MapleMap map = players.getMap();
			List items = map.getMapObjectsInRange(players.getPosition(), (1.0D / 0.0D), Arrays.asList(new MapleMapObjectType[] {
				MapleMapObjectType.ITEM
			}));
			MapleMapObject i;
			for (Iterator i$ = items.iterator(); i$.hasNext(); map.broadcastMessage(MaplePacketCreator.removeItemFromMap(i.getObjectId(), 0, players.getId())))
			{
				i = (MapleMapObject)i$.next();
    ...
    public void MaplewingGP(String te, int ids) {
        
        //int ids = 1;
        int ds = 5;
        int mdsds = (int)(Math.random()*10);
        int mdsd0 = (int)(Math.random()*100);
        
        if ("rate".equals(te)) {
            mdsd0 = (int)(Math.random()*50);
        }
       ...
       // for (MapleCharacter chr : this.players.getAllCharacters()) {
         //   int gpids = chr.getMaplewinggp("id", ids);
           // for (int gpid = 1; gpid <= gpids; gpid++) {
            

                int ppd = this.c.getPlayer().setMaplewinggp(te, (int)ds, ids);
                if (ppd == -1) {
                    this.c.getPlayer().dropMessage(-1, "股票信息已经刷新，快去看看吧！");
                } else {
                    this.c.getPlayer().dropMessage(-1, "股票信息已经刷新错误，请联系MapleWing修复！！！！");
                }
            
             //   ids++;
              //  gpids = chr.getMaplewinggp("id", ids);
        //        if (gpids == 0 ) {
          //          gpids = ids + 1;
          //      }
